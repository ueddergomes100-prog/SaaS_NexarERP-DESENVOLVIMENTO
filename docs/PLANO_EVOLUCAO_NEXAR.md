...
12. **Bug achado durante a validação do M4 (2026-08-15), ainda não corrigido:** a busca e o botão "Filtros" da tela principal **Estoque/Produtos** (`src/pages/Estoque/EstoqueList.tsx`, linha ~196) estão completamente desconectados — `<input>` sem `value`/`onChange`, botão "Filtros" sem `onClick`. Mesmo padrão do bug já corrigido em `CategoriasList.tsx` no Módulo 1 fatia 3. Não bloqueou a validação (foi contornado com scroll manual), mas é a tela de produtos mais usada do sistema — vale uma correção rápida e isolada.

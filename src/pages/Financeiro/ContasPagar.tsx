...
import { collection, query, onSnapshot, where, doc, updateDoc, addDoc, serverTimestamp, getDoc } from 'firebase/firestore';
import { db } from '../../services/firebase';
import { useAuth } from '../../contexts/AuthContext';
import { showSuccess, showError, NexusSwal } from '../../utils/alerts';
import { CheckCircle, Clock, Plus, X, ArrowDownCircle, Loader2, Calendar } from 'lucide-react';
...
const ContasPagar: React.FC = () => {
  ...
  const handleConciliar = async (t: TransacaoData) => {
    const result = await NexusSwal.fire({
      ...
      text: `Deseja confirmar o pagamento de R$ ${Number(t.valor).toFixed(2)} referente a ${t.descricao}?`,
      icon: 'question',
            ...
            <form onSubmit={handleSave} style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div className="input-group" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <input 
                  ...
                  style={{ backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', padding: '12px', color: 'white', textTransform: 'uppercase' }}
                  required
                />
              </div>

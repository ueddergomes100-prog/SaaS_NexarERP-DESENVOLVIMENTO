...
11. **Notas Recebidas automática via Spedy (achado em 2026-08-06, auditando a documentação da Spedy pedida pelo usuário):** a Spedy tem uma API que já busca automaticamente, via SEFAZ, as notas que fornecedores emitem contra o CNPJ da empresa (com manifestação e XML completo) — poderia substituir ou complementar o fluxo 100% manual do `EntradaNFE.tsx` de hoje (usuário baixa o XML e arrasta pro sistema). **Pendências antes de virar plano:** confirmar se o plano do usuário na Spedy tem o recurso `inbound_invoices` habilitado, e se há certificado digital A1 configurado lá (pré-requisito da própria Spedy). **Usuário pediu explicitamente pra ser lembrado desse item em toda continuação do trabalho até decidir** — sempre mencionar como pendência em aberto ao retomar o módulo fiscal ou o plano de evolução, não deixar cair.

...
Typecheck/lint (0 erros, 1 aviso pré-existente sem relação)/133 testes/build todos limpos. **Falta validação manual ao vivo** — bloqueada nesta sessão porque o tenant de dev tem `spedyEnabled:true` mas sem `spedyApiKey` configurada (a tela mostra "Módulo Fiscal Desativado" em vez do conteúdo), e não há nenhuma nota fiscal cadastrada ainda pra esse tenant. Tentativa de só *ler* o valor bruto de `spedyApiKey` (pra restaurar com precisão depois de um teste temporário) foi bloqueada pelo classificador de segurança do Claude Code — campo de API key é tratado como sensível, não contornado. **Retomar validação:** usuário configura a chave sandbox da Spedy em Configurações (nunca a Claude digitando), depois criar notas de teste sintéticas (mesma técnica do XML sintético do F22) pra exercitar paginação (>20 notas), WhatsApp (cliente com telefone vs. sem), gate de permissão de Cancelar, e conferir o log de auditoria em `empresas/{tenantId}/logs` após cancelar uma nota de teste.
...
| M6 Central de Notas Fiscais (listagem) | 2 | 🟨 Código pronto (achado que já existia ~90%, só faltavam 5 lacunas — todas fechadas) — falta validação manual, bloqueada por falta de chave Spedy no tenant de dev | 2026-08-15 |
